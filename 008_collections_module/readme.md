...
The keyword argument module is used to update the  \_\_module\_\_ data model identifier. This is consistent to the name of the Python script file (script.py file) that the NamedTuple subclass DateTuple is assigned in:
...
Recall the formal string representation uses the \_\_repr\_\_ data model identifier and the informal string representation uses the \_\_str\_\_ data model identifier which are more typically invoked using Pythons builtins function repr and str respectively:
...
If the directory function dir is used, all the identifiers are identical to that of the dict class with the exception to \_\_missing\_\_ which is added:
