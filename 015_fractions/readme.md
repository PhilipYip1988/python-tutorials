...
The datamodel ```__add__``` is setup so addition is carried out using the ```+``` operator:
...
The datamodel ```__sub__``` is setup for fraction addition using the ```-``` operator:
...
The datamodel ```__mul__``` is setup forfraction subtraction using the ```*``` operator:
...
The datamodel ```__div__``` is setup for fraction division using the ```/``` operator:
...
The datamodel ```__floordiv__``` is setup for integer fraction division using the ```//``` operator:
...
$$\text{f2}\%\text{f1}=\frac{1}{2}\%\frac{3}{8}=\frac{1\ast8\%3\ast2}{2\ast8}=\frac{8\%6}{16}=\frac{2}{16}=\frac{1}{8}$$